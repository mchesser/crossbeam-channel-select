...
enum SelectType {
    Recv { receiver: Expr, result: Ident, body: Expr },
    RecvAny { receiver: Expr, result: Ident, index: Ident, body: Expr },
    Send { sender: Expr, message: Expr, result: Ident, body: Expr  },
    SendAny { sender: Expr, message: Expr, result: Ident, index: Ident, body: Expr },
}
...
fn parse_selection(input: &ParseStream) -> Result<SelectType> {
    ...
    if lookahead.peek(kw::recv) {
        ...
        Ok(SelectType::Recv { receiver, result, body })
    }
        ...
        Ok(SelectType::RecvAny { receiver, result, index, body })
    }
        ...
        Ok(SelectType::Send { sender, message, result, body })
    }
        ...
        Ok(SelectType::SendAny { sender, message, result, index, body })
    }
    else {
        Err(lookahead.error())
    }
...
pub fn dynamic_select(input: TokenStream) -> TokenStream {
    ...
    for (index, selection) in input.selections.into_iter().enumerate() {
        ...
        match selection {
            SelectType::Recv { receiver, result, body } => {
            ...
            SelectType::RecvAny { receiver, result, index, body } => {
            ...
            SelectType::Send { sender, message, result, body } => {
                selections.push(quote! {
            ...
            SelectType::SendAny { sender, message, index, result, body } => {
                let id_end = Ident::new(&format!("__sel_{}_end", index), Span::call_site());
